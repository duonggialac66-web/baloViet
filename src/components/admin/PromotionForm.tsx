...
export default function PromotionForm({ initialData, isEdit = false }: PromotionFormProps) {
  ...
  const parseInitialTransform = () => {
    ...
    if (!raw) return { x: 82, y: 50, scale: 1.0, isProductPng: false, themeColor: "auto" };
    try {
      if (raw.startsWith("{")) {
        ...
        <div className="flex justify-center w-full">
          ...
          <div className="relative z-10 max-w-2xl space-y-4 pointer-events-none my-auto">
            ...
            <h1
              ...
              className={`font-display font-black text-3xl sm:text-5xl lg:text-6xl uppercase tracking-tight leading-[0.95] ${
                colorInfo.isLight ? "drop-shadow-[0_2px_8px_rgba(255,255,255,0.9)]" : "drop-shadow-[0_4px_16px_rgba(0,0,0,0.9)]"
              }`}
            >
              {formData.title ? (
